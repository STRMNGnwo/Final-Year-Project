...
##This class is to be used if images have not been split yet.
class CTADataset(torch.utils.data.Dataset):
        ...
        def get_target_map_from_mask(self,maskSlice):
            ...
            # target_map=np.stack(target_map,axis=-1)

            #arg_maxing target map

            #return target_map #one-hot encoded
        ...
        def __getitem__(self,index):
            ...
            maskSlice_copy=functional.resize(img=maskSlice_copy,size=[128,128]) #NEEDS TO BE CHANGED MANUALLY
            ...
            if(self.transform):
                return self.transform(imageSlice),self.transform(maskSlice),imagePath,maskPath,target_map
        
            return imageSlice,maskSlice,imagePath,maskPath,target_map #would need to return the mask slice and image slice for the given patient.
